...
### Prompt 2 - Recode
Recode the variables `black`, `callback` and `crimrec` into a new dataframe `clean_p_exam`  (recode the `0` and `1`s)
...
criminalrecord_c %>% 
  ...
  ggplot(aes(x=callback_c, y= ..prop.., group=1))+
  geom_bar(fill="steelblue", width=0.5)+
  ggtitle("Callback for all applications in the Study") +
...
criminalrecord_c %>%
  ...
  kable() %>%
    kable_styling(bootstrap_options = "striped", 
                  full_width = T, 
                  position="left") %>% 
    add_header_above(c("","Did applicant receive callback?"= 2))  ### response variable
```
...
criminalrecord_c %>%
  ...
  ggplot(aes(x=crimrec_c, fill=callback_c)) +  ### x=explanatory, fill=response
    geom_bar(position = position_fill(reverse = F), width=.6) +
